...
If tools fail with `password authentication failed for user "supabase_read_only_user"` when the MCP server runs in read-only mode (`/mcp?read_only=true`), the `supabase_read_only_user` role has no password. New installations set it to `POSTGRES_PASSWORD` automatically. On existing installations, set it manually:

```sh
docker compose exec db psql -U supabase_admin -d postgres \
  -c "ALTER USER supabase_read_only_user WITH PASSWORD '<your POSTGRES_PASSWORD>';"
```
