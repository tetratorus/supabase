...
- Review the table privileges granted to `anon` and `authenticated`. RLS policies don't apply to `truncate`, so a role that keeps that grant can empty a table even when policies block `delete`. See [Grants and policies](/docs/guides/database/postgres/row-level-security#grants-and-policies).
