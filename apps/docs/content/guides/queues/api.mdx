...
Messages are returned but not removed from the Queue. Delete or archive each Message after processing it; `pgmq_public` doesn't expose a batch delete function. If you want to consume and delete a single Message in one call, use [`pop`](#pgmqpublicpopqueuename) instead.
