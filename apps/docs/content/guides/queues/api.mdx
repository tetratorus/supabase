...
Reads up to `n` Messages from the specified Queue and hides them from other consumers for `sleep_seconds`. All three parameters are required.

- `queue_name` (`text`): Queue name
- `sleep_seconds` (`integer`): Visibility timeout in seconds. Read Messages stay hidden from other consumers for this long, so choose a value longer than the time you need to process and [delete](#pgmqpublicdeletequeuename-messageid) or [archive](#pgmqpublicarchivequeuename-messageid) them. If a Message isn't deleted or archived before the timeout expires, it becomes visible again and can be read by another consumer. Pass `0` to read without hiding the Messages.
- `n` (`integer`): Maximum number of Messages to read

Messages are returned but not removed from the Queue. Delete or archive each Message individually after processing it. `pgmq_public` intentionally doesn't expose a batch delete function: deleting by a large list of Message IDs can bypass the `message_id` index and trigger a sequential scan of the whole Queue table. If you want to consume and delete a single Message in one call, use [`pop`](#pgmqpublicpopqueuename) instead.
