...
<Admonition type="note" title="Prisma version">

This guide targets Prisma v7 and later, where connection URLs live in the Prisma config file and Prisma Client requires a [driver adapter](https://www.prisma.io/docs/orm/overview/databases/database-drivers).

On Prisma v6 and earlier, set the URLs in the `datasource` block of `schema.prisma` instead:
...
For a serverless deployment, add `directUrl = env("DIRECT_URL")` so migrations run over a direct connection. Leave it out otherwise — Prisma resolves `env()` when it loads the datasource and errors if the variable isn't set.

If your v6 project has a `prisma.config.ts`, its `datasource.url` takes precedence over the schema block, so set the migration URL there or delete the file.

Otherwise follow this guide as written, with these changes: skip step 4, skip the `@prisma/adapter-pg` and `pg` installs in step 2, and in step 6 keep the generated-client import but drop the `PrismaPg` import, the adapter, and the `adapter` argument.

To move an existing project onto v7, follow [Prisma's upgrade guide](https://www.prisma.io/docs/orm/more/upgrade-guides/upgrading-versions/upgrading-to-prisma-7).
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={4}>
    <StepHikeCompact.Details title="Configure your Prisma config file">

    `prisma init` generates the config file next to your `package.json`. Recent v7 releases name it `prisma7.config.ts` and earlier ones name it `prisma.config.ts`; edit whichever file your CLI generated. The examples below show `prisma7.config.ts`.

    Make sure it starts with `import "dotenv/config"` so your `.env` file is read. If you are using a serverless environment, change the data source URL to `DIRECT_URL`, because the Prisma CLI uses it to run migrations.
