...
Otherwise follow this guide as written, with these changes: skip step 4, skip the `@prisma/adapter-pg` and `pg` installs in step 2, and construct `new PrismaClient()` from `@prisma/client` without the `adapter` argument in step 6.
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={4}>
    ...
    `prisma init` generates the config file next to your `package.json`, named `prisma7.config.ts` on Prisma 7.10 and `prisma.config.ts` on earlier releases. The CLI loads either name, preferring `prisma7.config.*`.
