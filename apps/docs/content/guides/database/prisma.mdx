...
On Prisma v6 and earlier, there is no config file: set the URLs in the `datasource` block of `schema.prisma` instead, and instantiate `PrismaClient` from `@prisma/client` without an adapter.

```prisma prisma/schema.prisma
datasource db {
  provider  = "postgresql"
  url       = env("DATABASE_URL")
  directUrl = env("DIRECT_URL") // serverless deployments only
}
```
