...
On Prisma v6 and earlier, set the URLs in the `datasource` block of `schema.prisma` instead, and instantiate `PrismaClient` from `@prisma/client` without an adapter:
...
If your v6 project has a `prisma.config.ts` (created by `prisma init` on recent v6 releases), its `datasource.url` takes precedence over the schema block, so set the migration URL there or delete the file.
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={4}>
    ...
    `prisma init` generates the config file next to your `package.json`, named `prisma7.config.ts` on recent v7 releases and `prisma.config.ts` on earlier ones. Prisma loads either name.
