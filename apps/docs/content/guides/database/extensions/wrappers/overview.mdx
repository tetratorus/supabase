...
Foreign tables should live in a private schema and are not accessible through the Data API directly. Query them with SQL, or expose them to your application through a database function. See [Security](#security) for a worked example.
