...
| --------------- | --------------------- | ------------------------------------------------------- |
| `anon`          | `all`                 | Only what signed-out visitors are meant to read         |
| `authenticated` | `all`                 | Only the operations your app exposes to signed-in users |
| `service_role`  | `all`                 | Full access. It bypasses RLS, so keep it server-side    |
...
The `all` grant includes every table privilege. Beyond `select`, `insert`, `update`, and `delete`, it grants `truncate`, `references`, `trigger`, and, on Postgres 17 and later, `maintain`. Policies don't apply to `truncate`. On a table with RLS enabled and no policies, `delete` removes nothing, but `truncate` empties the table. `revoke all` removes that privilege along with the rest.
