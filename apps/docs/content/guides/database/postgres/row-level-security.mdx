...
The `all` grant covers every table privilege: `select`, `insert`, `update`, `delete`, `truncate`, `references`, and `trigger`. Policies don't apply to `truncate`. On a table with RLS enabled and no policies, `delete` removes nothing, but `truncate` empties the table. `revoke all` removes that privilege along with the rest.
