...
The automatic grant is `all privileges`, so it also includes `truncate`, `references`, and `trigger`. Policies don't apply to `truncate`: on a table with RLS enabled and no policies, `delete` removes nothing, but `truncate` empties the table. `revoke all` removes that privilege along with the rest.
