...
On existing projects, tables created in `public` receive `ALL` privileges (not only `SELECT`, `INSERT`, `UPDATE`, and `DELETE`, but also `TRUNCATE`, `REFERENCES`, `TRIGGER`, and, on Postgres 17 and later, `MAINTAIN`) for `anon`, `authenticated`, and `service_role` by default. Functions receive `EXECUTE`. These grants make new objects reachable through the Data API, even when you don't intend to expose them.
