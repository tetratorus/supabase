...
- **Email confirmation, magic links, and password recovery** use `app/auth/confirm/route.ts`. Point your email templates at this URL. The handler calls `verifyOtp` with the `token_hash` in the URL. See [Password-based Auth](/docs/guides/auth/passwords) for the code.
