...
To access Supabase from a SvelteKit app, you need 2 types of Supabase clients:

1. **Browser client** - To access Supabase from Svelte components and universal `load` functions running in the browser.
2. **Server client** - To access Supabase from server `load` functions, form actions, server endpoints, and remote functions, which run only on the server.

The server client reads the current request with [`getRequestEvent`](https://svelte.dev/docs/kit/$app-server#getRequestEvent) from `$app/server`, so you can call `createClient()` anywhere on the server without passing the event around or storing the client in `event.locals`. `getRequestEvent` requires SvelteKit 2.20 or later.

<$Partial path="auth_methods.mdx" />

Create a `src/lib/supabase` folder with a file for each type of client. Then copy the utility functions for each client type.

<div className="mt-12">
  <$CodeTabs>
    <$CodeSample
      path="/auth/sveltekit/src/lib/supabase/client.ts"
      meta="name=src/lib/supabase/client.ts"
      language="typescript"
    />
    <$CodeSample
      path="/auth/sveltekit/src/lib/supabase/server.ts"
      meta="name=src/lib/supabase/server.ts"
      language="typescript"
    />
  </$CodeTabs>
</div>
...
Import the server client in server-only code and the browser client in components:
...
- Set up a Supabase server client utility for server `load` functions, form actions, server endpoints, and remote functions.
- Set up a Supabase browser client utility for Svelte components.
