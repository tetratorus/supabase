...
Skipping these tables does not lose your encrypted column data, as long as you copy the root encryption key to the new project as described in step 5 of the restore. Vault secrets are not carried over by the dump, so re-create them in the new project with [`vault.create_secret()`](/docs/guides/database/vault).
