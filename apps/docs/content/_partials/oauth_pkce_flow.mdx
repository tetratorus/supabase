...
The server-side callback below requires a client from `@supabase/ssr` on both the sign-in and callback sides. These clients use the PKCE flow and store the code verifier in cookies, so the callback can complete the exchange. A client created with `createClient` from `@supabase/supabase-js` defaults to the [implicit flow](/docs/guides/auth/sessions/implicit-flow), which redirects to your callback with the tokens in the URL fragment (`#access_token=...`) instead of a `code` query parameter. Setting `flowType: 'pkce'` on such a client only works when the same client completes the exchange, for example in the browser with `detectSessionInUrl` or by calling `exchangeCodeForSession`, because the [code verifier is stored locally](/docs/guides/auth/sessions/pkce-flow#limitations).
