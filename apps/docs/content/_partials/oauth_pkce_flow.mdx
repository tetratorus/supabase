...
The clients created by `@supabase/ssr` use the PKCE flow by default. If you create the client with `createClient` from `@supabase/supabase-js`, set `flowType: 'pkce'` in the `auth` options. The default is the [implicit flow](/docs/guides/auth/sessions/implicit-flow), which redirects to your callback with the tokens in the URL fragment (`#access_token=...`) instead of a `code` query parameter.
