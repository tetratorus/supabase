...
<RefSubLayout.EducationRow>
  <RefSubLayout.Details>
    ...
    Generating types for a hosted project with `--project-id` requires an authenticated CLI session, so run [`supabase login`](/docs/reference/cli/supabase-login) first if you haven't already. Login isn't needed when generating types from a local database (`--local`) or a direct connection string (`--db-url`). See the [full guide](/docs/guides/api/rest/generating-types#generating-types-using-supabase-cli) for details.
