...
| `POSTGRES_PASSWORD_READ_ONLY` | string | Self-hosted | Dedicated Postgres password for the `POSTGRES_USER_READ_ONLY` role. | Optional. Generated by `utils/generate-keys.sh`, applied by `volumes/db/roles.sql` on first start and rotated by `utils/db-passwd.sh`. Must differ from `POSTGRES_PASSWORD`. When empty, the role stays passwordless and read-only MCP mode returns a configuration error. |
| `POSTGRES_PORT` | integer | Self-hosted | Postgres TCP port. | Default: `5432`. |
| `POSTGRES_USER_READ_ONLY` | string | | Postgres role used by the local MCP server when running in read-only mode. | Default: `supabase_read_only_user`. Authenticates with `POSTGRES_PASSWORD_READ_ONLY`, never with `POSTGRES_PASSWORD`. On installs created before `POSTGRES_PASSWORD_READ_ONLY` existed, assign it manually: `ALTER USER supabase_read_only_user WITH PASSWORD '<POSTGRES_PASSWORD_READ_ONLY>';`. |
